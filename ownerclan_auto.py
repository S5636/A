...
def _extract_base_price(page, L, code):
    ...
    나온 뒤 "택배배송 N원" 배송비 문구가 이어진다. 사이드바 광고에도 '~원'
    가격이 많아서 페이지 전체 텍스트에서 그냥 찾으면 엉뚱한 숫자를 집을
    위험이 커, "상품코드"~"택배배송" 사이 구간으로 좁혀서 그 안에서만
    찾는다. 끝 경계로 원래 "예상배송일자"를 썼었는데, 실제로 이 문구 자체가
    없는 상품(WB80215로 확인 - 택배배송 다음 바로 옵션 영역이고 예상배송일자
    줄이 없음)에서 가격을 통째로 못 찾는 사고가 있었다(사용자 지적: "왜 가격
    못가져오냐") - "택배배송"은 배송비 정보로 모든 상품에 공통으로 뜨는
    문구라 더 안전한 경계다. 혹시 이것도 없는 상품이 있을까봐 예상배송일자로
    한 번 더 시도해본다. 정가 취소선 없이 가격이 하나만 뜨는 상품도 있을 수
    있어 그 경우는 그 값을 쓴다. (멤버십 전용 추가할인가는 셀러가 별도로
    선점해야 적용되는 값이라 기본 판매가로 보지 않는다 - 실제 발주 시
    결제금액과 다르면 조정 필요.)"""
    try:
    ...
    if start == -1 or end == -1 or end <= start:
        L(f"[{code}] 가격이 있어야 할 화면 영역(상품코드~택배배송/예상배송일자)을 못 찾았습니다 - 가격 확인을 건너뜁니다.")
        return None
