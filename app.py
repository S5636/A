...
def init_db():
    ...
    conn.commit()
    conn.close()
    # 이미 확인된 건(stock_status가 채워진 행)은 STOCK 대상 선정 조건("칸이
    # 비어있는지")에서 계속 빠지니까, 매입예상가 계산 방식(배송비/묶음배송
    # 가능수량 반영)만 고쳐봤자 이미 예전 로직으로 저장된 옛날 값이 화면에
    # 그대로 남아있었다(사용자 지적: "이미 틀린값이 들어가있으니까
    # 스톡안하잖아"). 배포 시점에 이미 bundle_max_qty 컬럼이 추가돼 있을 수도
    # 있어서(직전 배포에서 막 추가됨) 컬럼 추가 여부로는 이 순간을 구분할 수
    # 없다 - 그래서 별도 마커 파일로 "이번 재계산 로직 배포 이후 한 번은
    # 전부 지웠는지"를 딱 한 번만 기록해서, 그 전까지 확인된 값은 전부
    # 지우고 다음 STOCK 실행 때 새 로직으로 다시 확인되게 한다.
    recalc_marker = os.path.join(BASE_DIR, '.stock_recalc_v2_done')
    if not os.path.exists(recalc_marker):
        ...
        conn2.close()
        try:
            with open(recalc_marker, 'w', encoding='utf-8') as f:
                f.write('done')
        except Exception:
            pass
