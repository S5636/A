...
def init_db():
    ...
    # 가능수량 반영, 가격 추출 구간 조정 등)만 고쳐봤자 이미 예전 로직으로
    # 저장된 옛날 값이 화면에 그대로 남아있었다(사용자 지적: "이미 틀린값이
    # 들어가있으니까 스톡안하잖아" / "수정하기전엔 그나마 가격을 가져왔었는데
    # 지금은 또 실패네" - 가격이 비어있지 않고 '틀리게' 남아있는 경우는 위의
    # est_buy_price 빈값 안전망으로도 못 걸러진다). 배포 시점에 이미 새
    # 컬럼이 추가돼 있을 수도 있어서(직전 배포에서 막 추가됨) 컬럼 추가
